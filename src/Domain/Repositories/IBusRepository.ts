...
export interface IBusRepository {
  ...
  obtenerAsientosDisponibles(busId: number, tipo: 'NORMAL' | 'VIP'): Promise<BusAsiento[]>;
  ...
  buscarDisponibles(ciudadOrigen: string, ciudadDestino: string, fecha: Date, filtros?: BusFiltros): Promise<Bus[]>;
}